...
```bash
Qdrant-App$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
Qdrant-App$ sam local start-api
Qdrant-App$ curl http://localhost:3000/classify_digit
```
...
```bash
Qdrant-App$ sam logs -n InferenceFunction --stack-name Qdrant-App --tail
```
...
```bash
aws cloudformation delete-stack --stack-name Qdrant-App
```
